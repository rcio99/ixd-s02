...
function portada() {
    createSpan("Estás en el inicio").parent("title");
    createElement('p','Mi color favorito es '+random(lorenIpsum)).parent(loPrincipal);
    select("a:nth-child(1)").style("font-weight", "bold").style("color", "#000000");
}

function pagina() {
    createSpan("Estás en la página 01").parent("title");
    createElement('p','Mi color favorito es '+random(lorenIpsum)).parent(loPrincipal);
